...
function extractFontFaceData(css, family) {
  const fontFaces = [];
  for (const node of cssTree.findAll(cssTree.parse(css), (node2) => node2.type === "Atrule" && node2.name === "font-face")) {
...
const mini$fetch = (url, options) => {
  if (options?.baseURL) {
    url = ufo.joinURL(options.baseURL, url);
  }
  if (options?.query) {
    url = ufo.withQuery(url, options.query);
  }
  return proxy.fetch(url, options).then((r) => options?.responseType === "json" ? r.json() : options?.responseType === "arrayBuffer" ? r.arrayBuffer() : r.text());
};
const $fetch = Object.assign(mini$fetch, {
  create: (defaults) => (url, options) => mini$fetch(url, defu__default(options, defaults))
});
...
const google = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`google:${fontFamily}-${ohash.hash(defaults)}-data.json`, () => getFontDetails$5(fontFamily, defaults), {
        onError(err) {
...
let fonts$5;
async function fetchFontMetadata$1() {
  return await $fetch("https://fonts.google.com/metadata/fonts", { responseType: "json" }).then((r) => r.familyMetadataList);
}
async function initialiseFontMeta$5() {
  fonts$5 = await cachedData("google:meta.json", fetchFontMetadata$1, {
    onError() {
...
function isGoogleFont(family) {
  return fonts$5.some((font) => font.family === family);
}
const styleMap = {
  italic: "1",
  oblique: "1",
...
async function getFontDetails$5(family, variants) {
  const font = fonts$5.find((font2) => font2.family === family);
  ...
  let css = "";
  for (const extension in userAgents$1) {
    css += await $fetch("/css2", {
      baseURL: "https://fonts.googleapis.com",
      headers: { "user-agent": userAgents$1[extension] },
      query: {
...
const googleicons = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`googleicons:${fontFamily}-${ohash.hash(defaults)}-data.json`, () => getFontDetails$4(fontFamily), {
        onError(err) {
...
async function initialiseFontMeta$4() {
  fonts$4 = await cachedData("googleicons:meta.json", fetchFontMetadata, {
    onError() {
...
function isGoogleIcon(family) {
  return fonts$4.includes(family);
}
async function getFontDetails$4(family) {
  let css = "";
...
const bunny = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`bunny:${fontFamily}-${ohash.hash(defaults)}-data.json`, () => getFontDetails$3(fontFamily, defaults), {
        onError(err) {
...
const fontshare = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`fontshare:${fontFamily}-${ohash.hash(defaults)}-data.json`, () => getFontDetails$2(fontFamily, defaults), {
        onError(err) {
...
const adobe = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`adobe:${fontFamily}-${ohash.hash(defaults)}-data.json`, () => getFontDetails$1(fontFamily, defaults), {
        onError(err) {
...
const fontsource = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`fontsource:${fontFamily}-${ohash.hash(defaults)}-data.json`, () => getFontDetails(fontFamily, defaults), {
        onError(err) {
...
async function getFontDetails(family, variants) {
  ...
  for (const subset of subsets) {
    for (const style of styles) {
      if (font.variable) {
        const variableAxes = await cachedData(`fontsource:${font.family}-axes.json`, () => fontAPI(`/variable/${font.id}`, { responseType: "json" }), {
          onError() {
...
async function resolveProviders(_providers = {}) {
  const nuxt = kit.useNuxt();
  const _jiti = jiti__default(nuxt.options.rootDir, { interopDefault: true });
  ...
  for (const key in providers) {
    ...
    if (typeof value === "string") {
      providers[key] = await _jiti(await kit.resolvePath(kit.resolveAlias(value)));
    }
  }
  return providers;
}
